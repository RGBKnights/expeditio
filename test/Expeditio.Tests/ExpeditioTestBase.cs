...
using Expeditio.Migrations.SeedData;
...
namespace Expeditio.Tests
{
    public abstract class ExpeditioTestBase : AbpIntegratedTestBase<ExpeditioTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected ExpeditioTestBase()
        {
        ...
        protected void UsingDbContext(Action<ExpeditioDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ExpeditioDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ExpeditioDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ExpeditioDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ExpeditioDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ExpeditioDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ExpeditioDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ExpeditioDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ExpeditioDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ExpeditioDbContext, Task<T>> func)
        {
            T result;
